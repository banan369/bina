...
- رفع وتغيير وحذف الصورة الشخصية عبر bucket باسم `avatars` مع حفظ رابطها في `profiles.avatar_url`.

## إعداد صور المستخدمين في Supabase

شغّلي ملف [`supabase.sql`](supabase.sql) يدويًا مرة واحدة من **Supabase Dashboard → SQL Editor** قبل تجربة رفع الصور. ينشئ الملف bucket عامًا للقراءة باسم `avatars` (حتى تظهر الصور في المنشورات)، ويقصر الرفع والتعديل والحذف على مجلد المستخدم الذي يطابق `auth.uid()`، مع حد 5 ميجابايت وصيغ JPG وPNG وWEBP. لا ينشئ الملف جدول `profiles` ولا يغيّر جداول الطلبات أو العروض أو الرسائل.
